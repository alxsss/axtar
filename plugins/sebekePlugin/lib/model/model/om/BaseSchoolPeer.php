...
abstract class BaseSchoolPeer {
	...
	private static $fieldNames = array (
		...
		BasePeer::TYPE_COLNAME => array (self::ID, self::NAME, self::VILLAGE_ID, ),
	...
	private static $fieldKeys = array (
		...
		BasePeer::TYPE_COLNAME => array (self::ID => 0, self::NAME => 1, self::VILLAGE_ID => 2, ),
	...
	public static function getMapBuilder()
	{
		if (self::$mapBuilder === null) {
			self::$mapBuilder = new SchoolMapBuilder();
		}
    ...
    foreach (sfMixer::getCallables('BaseSchoolPeer:doCount:doCount') as $callable)
    {
      call_user_func($callable, 'BaseSchoolPeer', $criteria, $con);
    }
    ...
    foreach (sfMixer::getCallables('BaseSchoolPeer:doSelectStmt:doSelectStmt') as $callable)
    {
      call_user_func($callable, 'BaseSchoolPeer', $criteria, $con);
    }
	...
	public static function doCountJoinVillage(Criteria $criteria, $distinct = false, PropelPDO $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		$criteria->addJoin(array(SchoolPeer::VILLAGE_ID,), array(VillagePeer::ID,), $join_behavior);
	 ...
	 *		 rethrown wrapped into a PropelException.
	 */
	public static function doSelectJoinVillage(Criteria $c, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{

    foreach (sfMixer::getCallables('BaseSchoolPeer:doSelectJoin:doSelectJoin') as $callable)
    {
      call_user_func($callable, 'BaseSchoolPeer', $c, $con);
    }
		...
		$c->addJoin(array(SchoolPeer::VILLAGE_ID,), array(VillagePeer::ID,), $join_behavior);
    ...
    foreach (sfMixer::getCallables('BaseSchoolPeer:doSelectJoinAll:doSelectJoinAll') as $callable)
    {
      call_user_func($callable, 'BaseSchoolPeer', $c, $con);
    }
		...
		VillagePeer::addSelectColumns($c);
		$startcol3 = $startcol2 + (VillagePeer::NUM_COLUMNS - VillagePeer::NUM_LAZY_LOAD_COLUMNS);
  ...
  static public function getUniqueColumnNames()
  {
    ...
    foreach (sfMixer::getCallables('BaseSchoolPeer:doInsert:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseSchoolPeer', $values, $con);
      if (false !== $ret)
      {
        return $ret;
      }
    ...
    foreach (sfMixer::getCallables('BaseSchoolPeer:doInsert:post') as $callable)
    {
      call_user_func($callable, 'BaseSchoolPeer', $values, $con, $pk);
    }
    ...
    foreach (sfMixer::getCallables('BaseSchoolPeer:doUpdate:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseSchoolPeer', $values, $con);
      if (false !== $ret)
      {
		...
		if ($values instanceof Criteria) {
			...
			$selectCriteria->add(SchoolPeer::ID, $criteria->remove(SchoolPeer::ID), $comparison);
    ...
    foreach (sfMixer::getCallables('BaseSchoolPeer:doUpdate:post') as $callable)
    {
      call_user_func($callable, 'BaseSchoolPeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the school table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		} elseif ($values instanceof School) {
		...
		try {
			...
			// for more than one table or we could emulating ON DELETE CASCADE, etc.
			$con->beginTransaction();
			$affectedRows += SchoolPeer::doOnDeleteCascade($criteria, $con);
			...
			// invalidate objects in SchoolUserPeer instance pool, since one or more of them may be deleted by ON DELETE CASCADE rule.
	...
	protected static function doOnDeleteCascade(Criteria $criteria, PropelPDO $con)
	{
		...
		foreach ($objects as $obj) {
			...
			$c = new Criteria(SchoolUserPeer::DATABASE_NAME);
			
			$c->add(SchoolUserPeer::SCHOOL_ID, $obj->getId());
			$affectedRows += SchoolUserPeer::doDelete($c, $con);
		}
		return $affectedRows;
	}
...
//
// NOTE: This static code cannot call methods on the SchoolPeer class, because it is not defined yet.
// If you need to use overridden methods, you can add this code to the bottom of the SchoolPeer class:
//
// Propel::getDatabaseMap(SchoolPeer::DATABASE_NAME)->addTableBuilder(SchoolPeer::TABLE_NAME, SchoolPeer::getMapBuilder());
//
// Doing so will effectively overwrite the registration below.

Propel::getDatabaseMap(BaseSchoolPeer::DATABASE_NAME)->addTableBuilder(BaseSchoolPeer::TABLE_NAME, BaseSchoolPeer::getMapBuilder());
