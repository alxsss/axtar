...
abstract class BaseRegion extends BaseObject  implements Persistent {
  ...
  const PEER = 'RegionPeer';
	...
	/**
	 * @var        array Village[] Collection to store aggregation of Village objects.
	 */
	protected $collVillages;

	/**
	 * @var        Criteria The criteria used to select the current contents of collVillages.
	 */
	private $lastVillageCriteria = null;
	...
	public function hasOnlyDefaultValues()
	{
			...
			if (array_diff($this->modifiedColumns, array(RegionPeer::NAME))) {
				return false;
			}
	...
	/**
	 * Clears out the collVillages collection (array).
	 *
	...
	/**
	 * Initializes the collVillages collection (array).
	 *
	 ...
	 * related Villages from storage. If this Region is new, it will return
		...
		if ($this->collVillages === null) {
			if ($this->isNew()) {
			   $this->collVillages = array();
				...
				$criteria->add(VillagePeer::REGION_ID, $this->id);

				VillagePeer::addSelectColumns($criteria);
				$this->collVillages = VillagePeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				VillagePeer::addSelectColumns($criteria);
				if (!isset($this->lastVillageCriteria) || !$this->lastVillageCriteria->equals($criteria)) {
					$this->collVillages = VillagePeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastVillageCriteria = $criteria;
	...
	public function countVillages(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collVillages === null) {
			if ($this->isNew()) {
				...
				$count = VillagePeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	public function addVillage(Village $l)
	{
		...
		if (!in_array($l, $this->collVillages, true)) { // only add it if the **same** object is not already associated
			array_push($this->collVillages, $l);
			$l->setRegion($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collVillages) {
				foreach ((array) $this->collVillages as $o) {
