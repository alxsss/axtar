...
abstract class BaseAlbumPeer {
	...
	private static $fieldNames = array (
		BasePeer::TYPE_PHPNAME => array ('Id', 'Title', 'Description', 'Visibility', 'UserId', 'CreatedAt', ),
		BasePeer::TYPE_STUDLYPHPNAME => array ('id', 'title', 'description', 'visibility', 'userId', 'createdAt', ),
		BasePeer::TYPE_COLNAME => array (self::ID, self::TITLE, self::DESCRIPTION, self::VISIBILITY, self::USER_ID, self::CREATED_AT, ),
		BasePeer::TYPE_FIELDNAME => array ('id', 'title', 'description', 'visibility', 'user_id', 'created_at', ),
	...
	private static $fieldKeys = array (
		BasePeer::TYPE_PHPNAME => array ('Id' => 0, 'Title' => 1, 'Description' => 2, 'Visibility' => 3, 'UserId' => 4, 'CreatedAt' => 5, ),
		BasePeer::TYPE_STUDLYPHPNAME => array ('id' => 0, 'title' => 1, 'description' => 2, 'visibility' => 3, 'userId' => 4, 'createdAt' => 5, ),
		BasePeer::TYPE_COLNAME => array (self::ID => 0, self::TITLE => 1, self::DESCRIPTION => 2, self::VISIBILITY => 3, self::USER_ID => 4, self::CREATED_AT => 5, ),
		BasePeer::TYPE_FIELDNAME => array ('id' => 0, 'title' => 1, 'description' => 2, 'visibility' => 3, 'user_id' => 4, 'created_at' => 5, ),
		BasePeer::TYPE_NUM => array (0, 1, 2, 3, 4, 5, )
	);
	...
	public static function addSelectColumns(Criteria $criteria)
	{
		...
		$criteria->addSelectColumn(AlbumPeer::VISIBILITY);

		$criteria->addSelectColumn(AlbumPeer::USER_ID);
    ...
    foreach (sfMixer::getCallables('BaseAlbumPeer:doCount:doCount') as $callable)
    {
      call_user_func($callable, 'BaseAlbumPeer', $criteria, $con);
    }
    ...
    foreach (sfMixer::getCallables('BaseAlbumPeer:doSelectStmt:doSelectStmt') as $callable)
    {
      call_user_func($callable, 'BaseAlbumPeer', $criteria, $con);
    }
	...
	public static function doCountJoinsfGuardUser(Criteria $criteria, $distinct = false, PropelPDO $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		$criteria->addJoin(array(AlbumPeer::USER_ID,), array(sfGuardUserPeer::ID,), $join_behavior);
	...
	/**
	 * Selects a collection of Album objects pre-filled with their sfGuardUser objects.
	 * @param      Criteria  $c
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
    ...
    foreach (sfMixer::getCallables('BaseAlbumPeer:doSelectJoin:doSelectJoin') as $callable)
    {
      call_user_func($callable, 'BaseAlbumPeer', $c, $con);
    }
		...
		$c->addJoin(array(AlbumPeer::USER_ID,), array(sfGuardUserPeer::ID,), $join_behavior);
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			if ($key2 !== null) {
				...
				// Add the $obj1 (Album) to $obj2 (sfGuardUser)
				$obj2->addAlbum($obj1);
    ...
    foreach (sfMixer::getCallables('BaseAlbumPeer:doSelectJoinAll:doSelectJoinAll') as $callable)
    {
      call_user_func($callable, 'BaseAlbumPeer', $c, $con);
    }
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			if ($key2 !== null) {
				...
				// Add the $obj1 (Album) to the collection in $obj2 (sfGuardUser)
				$obj2->addAlbum($obj1);
  ...
  static public function getUniqueColumnNames()
  {
    ...
    foreach (sfMixer::getCallables('BaseAlbumPeer:doInsert:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseAlbumPeer', $values, $con);
      if (false !== $ret)
      {
        return $ret;
      }
    ...
    foreach (sfMixer::getCallables('BaseAlbumPeer:doInsert:post') as $callable)
    {
      call_user_func($callable, 'BaseAlbumPeer', $values, $con, $pk);
    }
    ...
    foreach (sfMixer::getCallables('BaseAlbumPeer:doUpdate:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseAlbumPeer', $values, $con);
      if (false !== $ret)
      {
    ...
    foreach (sfMixer::getCallables('BaseAlbumPeer:doUpdate:post') as $callable)
    {
      call_user_func($callable, 'BaseAlbumPeer', $values, $con, $ret);
    }
	...
	protected static function doOnDeleteCascade(Criteria $criteria, PropelPDO $con)
	{
		...
		foreach ($objects as $obj) {
			...
			$c->add(PhotoPeer::ALBUM_ID, $obj->getId());
			$affectedRows += PhotoPeer::doDelete($c, $con);
		}
		return $affectedRows;
	}
