...
abstract class BaseCountry extends BaseObject  implements Persistent {
  ...
  const PEER = 'CountryPeer';
	...
	/**
	 * @var        array Region[] Collection to store aggregation of Region objects.
	 */
	protected $collRegions;

	/**
	 * @var        Criteria The criteria used to select the current contents of collRegions.
	 */
	private $lastRegionCriteria = null;
	...
	public function setAddressFormatId($v)
	{
		...
		if ($this->address_format_id !== $v || $v === 0) {
	...
	public function hasOnlyDefaultValues()
	{
			...
			if (array_diff($this->modifiedColumns, array(CountryPeer::ID,CountryPeer::NAME,CountryPeer::ISO_CODE_2,CountryPeer::ISO_CODE_3,CountryPeer::ADDRESS_FORMAT_ID))) {
				return false;
			}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setIsoCode2($this->iso_code_2);

		$copyObj->setIsoCode3($this->iso_code_3);

		$copyObj->setAddressFormatId($this->address_format_id);
	...
	/**
	 * Clears out the collRegions collection (array).
	 *
	...
	/**
	 * Initializes the collRegions collection (array).
	 *
	 ...
	 * related Regions from storage. If this Country is new, it will return
		...
		if ($this->collRegions === null) {
			if ($this->isNew()) {
			   $this->collRegions = array();
				...
				$criteria->add(RegionPeer::COUNTRY_ID, $this->id);

				RegionPeer::addSelectColumns($criteria);
				$this->collRegions = RegionPeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				RegionPeer::addSelectColumns($criteria);
				if (!isset($this->lastRegionCriteria) || !$this->lastRegionCriteria->equals($criteria)) {
					$this->collRegions = RegionPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastRegionCriteria = $criteria;
	...
	public function countRegions(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collRegions === null) {
			if ($this->isNew()) {
				...
				$count = RegionPeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	public function addRegion(Region $l)
	{
		...
		if (!in_array($l, $this->collRegions, true)) { // only add it if the **same** object is not already associated
			array_push($this->collRegions, $l);
			$l->setCountry($this);
		}
	}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Country has previously been saved, it will retrieve
	 * related sfGuardUserProfiles from storage. If this Country is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collsfGuardUserProfiles === null) {
			if ($this->isNew()) {
			   $this->collsfGuardUserProfiles = array();
				...
				$criteria->add(sfGuardUserProfilePeer::COUNTRY_ID, $this->id);
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collRegions) {
				foreach ((array) $this->collRegions as $o) {
