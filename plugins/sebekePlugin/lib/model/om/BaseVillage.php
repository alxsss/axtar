...
abstract class BaseVillage extends BaseObject implements Persistent
{
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            return $startcol + 3; // 3 = VillagePeer::NUM_HYDRATE_COLUMNS.
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = VillageQuery::create()
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(VillagePeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`ID`';
        }
        if ($this->isColumnModified(VillagePeer::NAME)) {
            $modifiedColumns[':p' . $index++]  = '`NAME`';
        }
        if ($this->isColumnModified(VillagePeer::REGION_ID)) {
            $modifiedColumns[':p' . $index++]  = '`REGION_ID`';
        }

        $sql = sprintf(
            'INSERT INTO `village` (%s) VALUES (%s)',
    ...
    {
        if (isset($alreadyDumpedObjects['Village'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Village'][$this->getPrimaryKey()] = true;
     ...
     * If this Village is new, it will return
    ...
    {
        $this->schoolsScheduledForDeletion = $this->getSchools(new Criteria(), $con)->diff($schools);

        foreach ($this->schoolsScheduledForDeletion as $schoolRemoved) {
            $schoolRemoved->setVillage(null);
        }
    ...
    public function addSchool(School $l)
    {
        ...
        if (!$this->collSchools->contains($l)) { // only add it if the **same** object is not already associated
            $this->doAddSchool($l);
        }
    ...
    {
        return (string) $this->exportTo(VillagePeer::DEFAULT_STRING_FORMAT);
    }
    ...
    public function __call($name, $params)
    {
        ...
        // symfony_behaviors behavior
        if ($callable = sfMixer::getCallable('BaseVillage:' . $name))
        {
