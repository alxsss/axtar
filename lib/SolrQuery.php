...
class SolrQuery {
	...
	// Example of a query -- different queries with their own logic should be made as separate functions
       function runQuery($titlekeywords, $start, $site='') {
    ...
    function fetchResults($query, $start, $site='')
    {
     ...
     if(empty($site))
     {
       $querystring = "/axtar?q=".trim(urlencode($query))."&start=".$start."&rows=".$this->limit."&version=2.2&indent=on";
     }
     else if($site=='site')
     {
       //$querystring = "q=".trim(urlencode($query))."&start=".$start."&rows=2&qt=site&version=2.2&indent=on";
       $querystring = "/site?q=".trim(urlencode($query))."&start=".$start."&rows=2&version=2.2&indent=on";
     }
     else 
     {
       //$querystring = "stylesheet=&q=".trim(urlencode($query))."&fq=site:".$site."&version=2.2&start=".$start."&rows=".$this->limit."&fl=id,title,url+score&qt=axtarsite";
       $querystring = "/axtarsite?stylesheet=&q=".trim(urlencode($query))."&fq=site:".$site."&version=2.2&start=".$start."&rows=".$this->limit."&fl=id,title,url+score";
     }
     */
      if($site=='axtar')
      {
